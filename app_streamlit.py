...
# Evita volver a entrenar el mismo modelo en cada interacción.
# Limita el uso de CPU para funcionar mejor en Streamlit Community Cloud.
# =========================================================

@st.cache_resource(show_spinner=False)
def entrenar_rf(data_modelo):
    ...
    X = data_modelo[variables_predictoras]
    y = data_modelo["Entrega_a_tiempo"]
...
@st.cache_resource(show_spinner=False)
def entrenar_elasticnet(data_modelo):
    variables = [
...
@st.cache_resource(show_spinner=False)
def entrenar_xgboost(data_modelo):
    variables = [
        "Shipping Mode", "Market", "Category Name", "Customer Segment", "Type",
    ...
    X = data_modelo[variables]
    y = data_modelo["Benefit per order"]
...
@st.cache_resource(show_spinner=False)
def entrenar_gbr(data_modelo):
    data_d4 = data_modelo.copy()
    ...
    data_d4["Cycle_Time"] = (
        data_d4["Shipping_Date"] - data_d4["Order_Date"]
    ).dt.days

    variables = [
        "Shipping Mode", "Market", "Order Region", "Category Name",
        "Customer Segment", "Days for shipment (scheduled)",
        "Order Item Quantity", "Sales", "Order Item Discount"
    ]
    categoricas = [
...
# -----------------------------
    ...
    with st.spinner("Cargando/entrenando Random Forest..."):
        pipeline_rf, X_test, y_test, y_pred, y_prob = entrenar_rf(data)
...
# -----------------------------
    ...
    with st.spinner("Cargando/entrenando ElasticNetCV..."):
        pipeline_en, X_test_d2, y_test_d2, y_pred_d2 = entrenar_elasticnet(data)
...
# -----------------------------
    ...
    with st.spinner("Cargando/entrenando XGBoost Regressor..."):
        pipeline_xgb, X_test_d3, y_test_d3, y_pred_d3 = entrenar_xgboost(data)
...
# -----------------------------
    ...
    with st.spinner("Cargando/entrenando Gradient Boosting Regressor..."):
        pipeline_gbr, X_test_d4, y_test_d4, y_pred_d4 = entrenar_gbr(data)
